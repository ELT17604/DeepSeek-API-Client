...
def get_api_key():
    api_key = os.environ.get("DEEPSEEK_API_KEY") or 'KEY_HERE_!!!!!!!!!!'
    if not api_key:
...
def list_and_select_model(client):
    ...
    try:
        ...
        if not available_models:
            ...
            return manual_model if manual_model else "deepseek-chat"
        ...
        manual_model = input("Enter model name (leave blank for 'deepseek-chat'): ")
        return manual_model if manual_model else "deepseek-chat"
...
def stream_chat(client, model_id):
    ...
    while True:
        try:
            ...
            print("Try again or type 'exit'/'new'.")
...
def main():
    ...
    print("!!!PALE API KEY ! INSIDER PREVIEW!!!")
    api_key = get_api_key()
    if not api_key:
    ...
    while True:
        ...
        if choice == '1':
            ...
            if not selected_model or selected_model == "null":
                clear_screen()
